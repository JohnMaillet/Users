...
```bash
User-Service$ sam local invoke NetCodeWebAPIServerless --event events/event.json
```
...
```bash
User-Service$ sam logs -n NetCodeWebAPIServerless --stack-name User-Service --tail
```
...
```bash
User-Service$ dotnet test test/ServerlessAPI.Tests/ServerlessAPI.Tests.csproj 
```
